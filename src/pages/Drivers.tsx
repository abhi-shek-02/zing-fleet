    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => { resetCarForm(); setEditCar(null); setShowCar(true); }}>
            <Plus className="mr-1 h-3.5 w-3.5" /> Car
          </Button>
          <Button size="sm" onClick={() => { resetDriverForm(); setEditDriver(null); setShowDriver(true); }}>
            <Plus className="mr-1 h-3.5 w-3.5" /> Driver
          </Button>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1">
          {cars.map(car => {
            ...
            return (
              <div key={car.id} className="rounded-md border p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => openEditCar(car)}>
      ...
      {selectedCar && (
        <div className="rounded-md border p-3 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold">{selectedCar.number} — Details</h2>
            <Button variant="ghost" size="sm" onClick={() => setSelectedCarId(null)} className="text-xs">Close</Button>
          </div>
          ...
          <Tabs defaultValue="costs">
            ...
            <TabsContent value="costs" className="mt-2 space-y-1">
              {carCosts.filter(c => c.carId === selectedCarId).length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-4">No costs recorded</p>
              ) : (
                carCosts.filter(c => c.carId === selectedCarId)
            ...
            return (
              <div key={driver.id} className="flex items-center justify-between rounded-md border p-3">
                ...
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="sm" onClick={() => openEditDriver(driver)}>
                    <Pencil className="h-3.5 w-3.5" />
                  </Button>
      ...
      <Drawer open={showCar} onOpenChange={(o) => { if (!o) { setShowCar(false); setEditCar(null); resetCarForm(); } else setShowCar(true); }}>
        <DrawerContent>
          ...
          <DrawerFooter>
            <Button onClick={saveCar}>{editCar ? "Update" : "Add"} Car</Button>
            <DrawerClose asChild><Button variant="outline">Cancel</Button></DrawerClose>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
      ...
      {/* Add Car Cost Drawer */}
      <Drawer open={showCarCost} onOpenChange={setShowCarCost}>
        <DrawerContent>
          ...
          <div className="space-y-3 px-4">
            ...
            <div><Label className="text-xs">Cost Type</Label>
              <Select value={ccType} onValueChange={v => setCcType(v as CarCost["costType"])}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="maintenance">Maintenance</SelectItem>
                  <SelectItem value="cf">CF (Fitness)</SelectItem>
          ...
          <DrawerFooter>
            <Button onClick={addCarCost} disabled={!ccAmount}>Save</Button>
            <DrawerClose asChild><Button variant="outline">Cancel</Button></DrawerClose>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
      ...
      {/* Add Car Doc Drawer */}
      <Drawer open={showCarDoc} onOpenChange={setShowCarDoc}>
        <DrawerContent>
          ...
          <DrawerFooter>
            <Button onClick={addCarDoc} disabled={!cdName}>Save</Button>
            <DrawerClose asChild><Button variant="outline">Cancel</Button></DrawerClose>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
